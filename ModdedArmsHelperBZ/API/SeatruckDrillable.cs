...
namespace ModdedArmsHelperBZ.API
{
    public class SeatruckDrillable : MonoBehaviour, IManagedUpdateBehaviour, IManagedBehaviour
    {
        ...
        private Seatruck drillingTruck;
        public float drillDamage { get; set; } = 10f;
        ...
        public void SetDrillDamage(float value)
        {
            Assembly assembly = Assembly.GetCallingAssembly();
            BZLogger.Log($"Received 'SetDrillDamage' message from [{assembly.GetName().Name}] with value: [{value}]");
            drillDamage = value;
        }
        ...
        public void OnDrill(Vector3 position, Seatruck seatruck, out GameObject hitObject)
        {
            float num = 0f;
            ...
            if (num > 0f)
            {
                ...
                drillable.health[num2] = Mathf.Max(0f, drillable.health[num2] - drillDamage);
        ...
        public void ManagedUpdate()
        {
            ...
            if (lootPinataObjects.Count > 0 && drillingTruck != null)
            {
                ...
                foreach (GameObject gameObject in lootPinataObjects)
                {
                    ...
                    {
                        Vector3 b = drillingTruck.mainCab.transform.position + new Vector3(0f, 0.8f, 0f);
                        ...
                        if (num < 3f)
                        {
                            ...
                            if (pickupable)
                            {                                
                                if (!drillingTruck.seatruckHelper.HasRoomForItem(pickupable))
                                {
                                    if (drillingTruck.seatruckHelper.IsPiloted())
                                    {
                                ...
                                else
                                {
                                    ...
                                    drillingTruck.seatruckHelper.GetRoomForItem(pickupable).UnsafeAdd(item);
