...
{
    public class SeatruckGrapplingHook : MonoBehaviour
    {
        public Rigidbody rb;
        ...
        {
            Seatruck seatruck = collisionInfo.gameObject.GetComponentInParent<Seatruck>();

            SeatruckGrapplingHook component = collisionInfo.gameObject.GetComponent<SeatruckGrapplingHook>();

            if (staticAttached || (fixedJoint && fixedJoint.connectedBody) || seatruck != null || component != null || IsPropulsionCannonAmmo(collisionInfo.gameObject))
            {
                return;
            }
