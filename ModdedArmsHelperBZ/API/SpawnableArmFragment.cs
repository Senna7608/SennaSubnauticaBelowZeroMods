...
namespace ModdedArmsHelperBZ.API
{
    public abstract class SpawnableArmFragment : ModPrefab_Fragment
    {
        ...
        protected override IEnumerator ModifyGameObjectAsync(IOut<bool> success)
        {
            ...
            TaskResult<bool> postModifyResult = new TaskResult<bool>();
            CoroutineTask<bool> postModifyRequest = new CoroutineTask<bool>(PostModifyGameObjectAsync(postModifyResult), postModifyResult);
            yield return postModifyRequest;
            if (!postModifyResult.Get())
            {
                BZLogger.Error("PostModifyGameObjectAsync failed!");
                yield break;
            }
